...
function particleBackground(){
  ...
  function animate(){
    ...
    particlesArray.forEach(p => {
      ...
      ctx.beginPath(); ctx.arc(p.x, p.y, p.size, 0, Math.PI * 2); ctx.fillStyle = color + '66'; ctx.fill();
    });
...
function typeWriter(el, texts){
  ...
  function type(){
    ...
    el.innerHTML = current.substring(0, j) + '<span style="border-right:3px solid var(--text)"></span>';
...
async function fetchAllProjects(){
  ...
  for(const user of DATA.githubs){
    try{
      ...
      const mapped = repos.map(repo => ({ title: repo.name.toUpperCase(), desc: repo.description || "No description.", img: 'https://opengraph.githubassets.com/1/' + user + '/' + repo.name, link: repo.html_url }));
...
function About(){ const statsHTML = DATA.stats.map(s => `<div class="glass-card" style="text-align:center"><h3 style="font-size:3rem;color:var(--text)">${s.number}+</h3><p style="font-weight:600">${s.label}</p></div>`).join(''); return `<section id="about"><div class="container"><h2 class="section-title">About Me</h2><div style="display:grid;grid-template-columns:1fr 2fr;gap:3rem;align-items:center"><img src="${DATA.foto}" style="width:100%;border-radius:20px;border:2px solid var(--text)"><div><p style="font-size:1.2rem;line-height:1.8">${DATA.about}</p><div class="stats-grid" style="margin-top:2rem">${statsHTML}</div></div></div></div></section>`; }
