...
function themeToggle(){ const btn = document.createElement('button'); btn.className = 'theme-toggle magnetic-btn'; btn.innerHTML = '🌙'; document.querySelector('nav').appendChild(btn); btn.onclick = function(){ const html = document.documentElement; if(html.getAttribute('data-theme') === 'dark'){ html.setAttribute('data-theme','light'); btn.innerHTML = '☀️'; } else{ html.setAttribute('data-theme','dark'); btn.innerHTML = '🌙'; } } }
...
async function fetchAllProjects(){ const projectsContainer = document.getElementById('projects-grid'); if(!projectsContainer) return; projectsContainer.innerHTML = '<p class="loading">Loading projects...</p>'; let allRepos = []; for(const user of DATA.githubs){ try{ const res = await fetch('https://api.github.com/users/' + user + '/repos?sort=updated&per_page=12'); const repos = await res.json(); const mapped = repos.map(repo => ({ title: repo.name.toUpperCase(), desc: repo.description || "No description. Click to view code.", img: 'https://opengraph.githubassets.com/1/' + user + '/' + repo.name, link: repo.html_url })); allRepos = allRepos.concat(mapped); }catch(e){console.log(e)} } const projectHTML = allRepos.map((p,i) => `<div class="project-card glass-card" style="transition-delay:${i*0.05}s"><img src="${p.img}" class="project-img" onerror="this.src='https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=600'"><h3 style="font-size:1.5rem;margin-bottom:1rem">${p.title}</h3><p style="color:var(--muted);font-size:1rem;margin-bottom:1.5rem;min-height:60px">${p.desc}</p><a href="${p.link}" target="_blank" class="magnetic-btn">View Code</a></div>`).join(''); projectsContainer.innerHTML = projectHTML; magneticButtons(); scrollReveal(); }
