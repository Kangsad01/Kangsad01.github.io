const DATA = {
  ...
  foto: "https://avatars.githubusercontent.com/Kangsad01", // UDAH GANTI KE KANGSAD01
...
function scrollReveal(){
  const observer = new IntersectionObserver((entries)=>{ entries.forEach(entry=>{ if(entry.isIntersecting){ entry.target.classList.add('revealed'); if(entry.target.querySelector('.counter')){ animateCounter(entry.target.querySelector('.counter')); } }) },{threshold:0.1});
...
function init(){
  injectCSS();
  liquidBackground();
  mouseGlow(); // FITUR BARU
  customCursor();
  scrollProgress();
  backToTop();
  document.body.innerHTML = Navbar() + Hero() + Terminal() + About() + Projects() + TechStack() + Footer(); // TAMBAH TERMINAL
