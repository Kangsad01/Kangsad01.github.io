const DATA = {
  ...
  tech: [ // GANTI SKILLS JADI TECH STACK + LOGO
...
app.innerHTML = `
  ...
  <div class="easter" id="easter">🎉 GG EZ 🎉</div>
...
function Navbar(){
  return `<nav id="navbar">
    ...
    <ul class="nav-links">
      ...
      <li><a href="#tech">Tech</a></li><li><a href="#projects">Work</a></li>
      <li><a href="#testimonials">Testi</a></li><li><a href="#contact">Contact</a></li>
      <li><a href="${DATA.social.github}" target="_blank"><i data-lucide="github"></i></a></li>
    ...
    <div class="mobile-menu" id="mobileMenu">
      <a href="#home" onclick="toggleMenu()"><i data-lucide="home"></i> Home</a>
      <a href="#about" onclick="toggleMenu()"><i data-lucide="user"></i> About</a>
      <a href="#tech" onclick="toggleMenu()"><i data-lucide="code"></i> Tech Stack</a>
      <a href="#projects" onclick="toggleMenu()"><i data-lucide="briefcase"></i> Work</a>
      <a href="#testimonials" onclick="toggleMenu()"><i data-lucide="message-square"></i> Testi</a>
      <a href="#contact" onclick="toggleMenu()"><i data-lucide="mail"></i> Contact</a>
      <a href="${DATA.social.github}" target="_blank"><i data-lucide="github"></i> Github</a>
      <a href="${DATA.social.instagram}" target="_blank"><i data-lucide="instagram"></i> Instagram</a>
...
// TECH STACK BARU
function TechStack(){
  return `<section id="tech" class="reveal">
    <h2 style="font-size:3rem">Tech <span class="gradient">Stack</span></h2>
    <div class="tech-grid">
      ${DATA.tech.map(t=>`
        <div class="tech-card magnetic">
          <img src="${t.icon}" alt="${t.name}">
          <h4>${t.name}</h4>
          <div class="tech-progress">
            <div class="tech-progress-fill" data-level="${t.level}"></div>
          </div>
        </div>
      `).join('')}
    </div>
...
function init(){
  ...
  document.addEventListener('keydown', e => {
    if(e.key === konami[konamiIndex]){ konamiIndex++; if(konamiIndex === konami.length){ document.getElementById('easter').style.display='block'; setTimeout(()=>document.getElementById('easter').style.display='none',2000); konamiIndex=0; } } else { konamiIndex=0; }
  })
  ...
  const techObs = new IntersectionObserver((entries)=>{
    entries.forEach(entry=>{
      if(entry.isIntersecting){
        document.querySelectorAll('.tech-progress-fill').forEach(bar=>{
          bar.style.width = bar.getAttribute('data-level') + '%';
        })
      }
    })
  },{threshold:0.3});
  techObs.observe(document.getElementById('tech'));
  ...
  window.addEventListener('scroll', () => { const scrollY = window.scrollY; if(parallaxImg){ parallaxImg.style.transform = `translateY(${scrollY * 0.2}px) rotateX(${scrollY * 0.05}deg)`; } });
  ...
  document.addEventListener('mousemove', e=>{ blob.style.left = e.clientX - 300 + 'px'; blob.style.top = e.clientY - 300 + 'px'; })
