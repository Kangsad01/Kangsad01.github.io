...
function particleBackground(){
  ...
  for(let i = 0; i < 60; i++){ particlesArray.push({ x: Math.random() * canvas.width, y: Math.random() * canvas.height, size: Math.random() * 2 + 1, speedX: Math.random() * 0.4 - 0.2, speedY: Math.random() * 0.4 - 0.2 }) } // 60 aja biar ringan
  ...
  function animate(){
    ...
    particlesArray.forEach(p => {
      ...
      if(dist < mouse.radius){ p.x -= dx/dist * 1; p.y -= dy/dist * 1; }
      p.x += p.speedX; p.y += p.speedY;
      if(p.x < 0 || p.x > canvas.width) p.speedX *= -1;
      if(p.y < 0 || p.y > canvas.height) p.speedY *= -1;
      ctx.beginPath(); ctx.arc(p.x, p.y, p.size, 0, Math.PI * 2); ctx.fillStyle = 'rgba(56,189,248,0.4)'; ctx.fill();
    });
...
function typeWriter(el, texts){
  ...
  function type(){
    ...
    el.innerHTML = current.substring(0, j) + '<span style="border-right:3px solid var(--accent)"></span>';
    if(!isDeleting && j < current.length){ j++; setTimeout(type, 80) }
    else if(isDeleting && j > 0){ j--; setTimeout(type, 40) }
    else if(!isDeleting && j === current.length){ isDeleting = true; setTimeout(type, 2000) }
    else { isDeleting = false; i = (i + 1) % texts.length; setTimeout(type, 500) }
  }
  type();
}
...
async function handleFormSubmit(e){
  ...
  try{ const res = await fetch(DATA.formspree, { method: 'POST', body: new FormData(form), headers: {'Accept': 'application/json'} });
...
function init(){
  ...
  particleBackground(); // BALIKIN LAGI
  scrollProgress();
  backToTop();
  themeToggle();
  typeWriter(document.getElementById('role-text'), DATA.role); // BALIKIN LAGI
  fetchAllProjects();
}
