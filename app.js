...
// ========== 2. INJECT CSS ==========
function injectCSS(){
  const css = `:root{--bg:#030303;--text:#f5f5f5;--muted:#a0a0a0;--card:rgba(17,17,17,0.6);--accent:#9333ea;--accent2:#06b6d4;--border:rgba(255,255,255,0.1)}[data-theme='light']{--bg:#f5f5f5;--text:#030303;--muted:#525252;--card:rgba(255,255,255,0.6);--border:rgba(0,0,0,0.1)}*{margin:0;padding:0;box-sizing:border-box}html{scroll-behavior:smooth}body{font-family:'Satoshi',sans-serif;background:var(--bg);color:var(--text);overflow-x:hidden}.progress-bar-top{position:fixed;top:0;left:0;height:3px;background:linear-gradient(90deg,var(--accent),var(--accent2));z-index:10001;transform-origin:left;transform:scaleX(0);transition:transform 0.1s}.cursor{position:fixed;width:8px;height:8px;background:var(--accent);border-radius:50%;pointer-events:none;z-index:9999}.cursor-follower{position:fixed;width:40px;height:40px;border:2px solid var(--accent);border-radius:50%;pointer-events:none;z-index:9998;transition:transform.3s ease-out, width.3s, height.3s}#app{opacity:0;transition:opacity.5s}#app.loaded{opacity:1}.loader{position:fixed;inset:0;background:var(--bg);z-index:9999;display:flex;align-items:center;justify-content:center;font-size:3rem;font-weight:900;transition:opacity.5s}.loader.hidden{opacity:0;pointer-events:none}.glitch{position:relative}.glitch::before,.glitch::after{content:attr(data-text);position:absolute;top:0;left:0;width:100%;height:100%}.glitch::before{animation:glitch-1 0.5s infinite; color:var(--accent); z-index:-1}.glitch::after{animation:glitch-2 0.5s infinite; color:var(--accent2); z-index:-2}@keyframes glitch-1{0%,100%{clip-path:inset(0 0 80% 0)}50%{clip-path:inset(10% 0 10% 0);transform:translate(-2px,2px)}}@keyframes glitch-2{0%,100%{clip-path:inset(80% 0 0 0)}50%{clip-path:inset(10% 0 10% 0);transform:translate(2px,-2px)}}nav{position:fixed;top:0;width:100%;padding:2rem 5%;display:flex;justify-content:space-between;align-items:center;background:rgba(3,3,3,0.1);backdrop-filter:blur(20px);z-index:1000;border-bottom:1px solid var(--border)}.logo{font-weight:900;font-size:1.5rem;letter-spacing:2px}.nav-links{display:flex;gap:2rem;list-style:none}.nav-links a{color:var(--muted);text-decoration:none}.nav-links a:hover{color:var(--text)}section{padding:10rem 5%;min-height:100vh}.section-title{font-size:3rem;font-weight:900;text-align:center;margin-bottom:4rem}.liquid-btn{padding:1rem 2.5rem;border-radius:50px;text-decoration:none;font-weight:700;background:linear-gradient(90deg,var(--accent),var(--accent2));color:#fff;border:none;cursor:pointer;transition:all.3s}.liquid-btn:hover{transform:scale(1.05);box-shadow:0 0 30px rgba(147,51,234,0.5)}.hero{text-align:center}.hero-title{font-size:5rem;font-weight:900;line-height:1}.hero-subtitle{font-size:1.3rem;color:var(--muted);margin:2rem 0;max-width:600px;margin-left:auto;margin-right:auto}.about-grid{display:grid;grid-template-columns:1fr 1fr;gap:4rem;align-items:center;max-width:1200px;margin:0 auto}.about-img{width:100%;border-radius:20px;border:2px solid var(--accent)}.stats-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:2rem;margin-top:3rem}.stat-box{text-align:center;padding:2rem;background:var(--card);border-radius:15px}.stat-box h3{font-size:2.5rem;color:var(--accent)}.tilt-card{background:var(--card);backdrop-filter:blur(20px);border:1px solid var(--border);border-radius:20px;transition:transform.3s}.project-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(350px,1fr));gap:2rem}.project-card{overflow:hidden}.project-img{width:100%;height:250px;object-fit:cover;border-radius:15px 15px 0 0}.project-content{padding:2rem}.project-tags{display:flex;gap:.5rem;flex-wrap:wrap;margin:1rem 0}.tag{border:1px solid var(--accent);color:var(--accent);padding:.3rem.8rem;border-radius:20px;font-size:.8rem}.tech-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:2rem}.tech-card{padding:2rem;text-align:center}.tech-card img{width:48px;height:48px;margin-bottom:1rem}.reveal{opacity:0;transform:translateY(50px);transition:all 1s}.reveal.active{opacity:1;transform:translateY(0)}@media(max-width:768px){.nav-links{display:none}.hero-title{font-size:3rem}.cursor,.cursor-follower{display:none}.about-grid{grid-template-columns:1fr}}`;
...
function init(){
  ...
  // FIX: hide loader setelah semua ke render
  setTimeout(()=>{
...
// PENTING: BIKIN LOADER DULU BARU JALANIN INIT
document.addEventListener('DOMContentLoaded', () => {
  const app = document.getElementById('app');
  if(app){ 
    app.innerHTML = `<div class="loader" id="loader"><div class="glitch" data-text="${DATA.nama}">${DATA.nama}</div></div>`; 
  }
  init(); // baru jalanin init setelah loader ada
});
