...
const DATA = {
  ...
  foto: "https://i.imgur.com/8QfZ8pL.jpg", // GANTI PAKE LINK FOTO KAMU DARI WA TADI
  ...
  projects: [
    ...
    {title: "DASHBOARD ANALYTICS", desc: "Admin dashboard modern dengan chart real-time, dark mode, dan CRUD lengkap.", tags: ["React", "Tailwind", "Chart.js"], link: "https://github.com/Kangsad01", img: "https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=600"},
    {title: "SAAS LANDING PAGE", desc: "Landing page untuk produk SaaS dengan animasi scroll-trigger dan form terintegrasi.", tags: ["Next.js", "Framer Motion"], link: "https://github.com/Kangsad01", img: "https://images.unsplash.com/photo-1551434678-e076c223a692?w=600"}
  ],
  blog: [], testimonials: [],
...
<section id="about" class="reveal">
  ...
  <div class="about-grid">
    <img src="${DATA.foto}" alt="Foto ${DATA.nama}" class="about-img tilt-card">
    <div>
      <h3 style="font-size:2rem;margin-bottom:1rem">Halo, saya ${DATA.nama}</h3>
      <p style="color:var(--muted);line-height:1.8">${DATA.about}</p>
      <div class="stats-grid">
        ${DATA.stats.map(s=>`<div class="stat-box"><h3>${s.number}+</h3><p>${s.label}</p></div>`).join('')}
      </div>
    </div>
  </div>
</section>`; }
...
function init(){
  ...
  app.innerHTML = Navbar() + Hero() + About() + Projects() + TechStack() + Contact() + Footer(); // <--- ABOUT DITAMBAH DISINI
  tiltEffect(); runObserver();
  let i=0; const roleEl = document.getElementById('role-text'); roleEl.innerText = DATA.role[0];
  setInterval(()=>{i=(i+1)%DATA.role.length; roleEl.innerText = DATA.role[i]}, 2500);
...
document.getElementById('app').innerHTML = `<div class="loader" id="loader"><div class="glitch" data-text="${DATA.nama}">${DATA.nama}</div></div>`;
