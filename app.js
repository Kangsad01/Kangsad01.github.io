...
function injectCSS(){
  const css = `
  @import url('https://fonts.googleapis.com/css2?family=Outfit:wght@400;700;900&family=Fira+Code:wght@400;700&display=swap');
  ...
  *{margin:0;padding:0;box-sizing:border-box} html{scroll-behavior:smooth}
  ...
  body::before{content:'';position:fixed;top:0;left:0;width:100%;height:100%;background:url('https://grainy-gradients.vercel.app/noise.svg');opacity:0.05;z-index:-1;pointer-events:none}
  #glow{position:fixed;width:400px;height:400px;border-radius:50%;background:radial-gradient(circle, rgba(236,72,153,0.15) 0%, transparent 70%);pointer-events:none;z-index:-1;transition:transform 0.2s}
...
.magnetic-btn{padding:1rem 2.5rem;border-radius:100px;font-weight:900;background:linear-gradient(90deg,#ec4899,#f472b6);color:#fff;text-decoration:none;border:none;cursor:pointer;box-shadow:0 0 30px rgba(236,72,153,0.5);display:inline-block;margin-top:1rem;transition:transform 0.2s}
...
.terminal{background:#000;border:1px solid var(--border);border-radius:15px;padding:2rem;font-family:'Fira Code',monospace;max-width:800px;margin:0 auto;box-shadow:0 0 50px rgba(236,72,153,0.2)}
.terminal-header{display:flex;gap:8px;margin-bottom:1.5rem}
...
.project-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:3rem}
...
.loading{text-align:center;font-size:2rem;color:var(--accent)}
  ...
  @media(max-width:768px){.about-grid,.project-grid,.tech-grid{grid-template-columns:1fr}.hero-title{font-size:3.5rem}.section-title{font-size:3rem}}
  `;
  document.head.appendChild(Object.assign(document.createElement('style'), {innerHTML: css}));
}

function mouseGlow(){
  const glow = document.createElement('div'); glow.id='glow'; document.body.appendChild(glow);
  document.addEventListener('mousemove', e=>{ glow.style.left = e.clientX - 200 + 'px'; glow.style.top = e.clientY - 200 + 'px'; })
}
function magneticButtons(){
  document.querySelectorAll('.magnetic-btn').forEach(btn=>{
    btn.addEventListener('mousemove', e=>{ const rect = btn.getBoundingClientRect(); const x = e.clientX - rect.left - rect.width/2; const y = e.clientY - rect.top - rect.height/2; btn.style.transform = `translate(${x*0.3}px, ${y*0.3}px)`; })
...
function terminalType(){
  ...
  const el = document.getElementById('terminal-text'); if(!el) return;
  let i=0, j=0; function type(){ if(j < lines[i].length){ el.innerHTML += lines[i][j++]; setTimeout(type, 50); } else { el.innerHTML += '<br>'; i++; j=0; if(i < lines.length) setTimeout(type, 800); } } type();
}
async function fetchAllProjects(){
  ...
  for(const user of DATA.githubs){
    try{ const res = await fetch(`https://api.github.com/users/${user}/repos?sort=updated&per_page=12`); const repos = await res.json(); const mapped = repos.map(repo => ({ title: repo.name.toUpperCase(), desc: repo.description || "No description. Click to view code.", img: `https://opengraph.githubassets.com/1/${user}/${repo.name}`, link: repo.html_url })); allRepos = [...allRepos,...mapped]; }catch(e){console.log(e)}
  }
  ...
  projectsContainer.innerHTML = projectHTML; magneticButtons(); scrollReveal();
}
...
function init(){ injectCSS(); liquidBackground(); mouseGlow(); customCursor(); scrollProgress(); backToTop(); document.body.innerHTML = Navbar() + Hero() + Terminal() + About() + Projects() + TechStack() + Footer(); typeWriter(document.getElementById('role-text'), DATA.role); fetchAllProjects(); setTimeout(()=>{scrollReveal(); terminalType();}, 500); }
