...
function injectCSS(){
 ...
 .typing-cursor{display:inline-block;width:3px;background:var(--accent);animation:blink 1s infinite}
  ...
  @media(max-width:768px){.about-grid{grid-template-columns:1fr}.stats-grid{grid-template-columns:1fr}.hero-title{font-size:3rem} }
  `;
...
function shootConfetti(){ 
  for(let i=0;i<50;i++){ 
    ...
    c.animate([{transform:'translateY(0)'},{transform:`translateY(${window.innerHeight}px) rotate(720deg)`}],{duration:2000}); 
